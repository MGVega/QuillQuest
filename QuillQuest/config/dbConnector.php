...
class dbConnector{
	...
	public function __construct(){
		...
		if(_ENVIRONMENT=='localhost'){
			$this->root = dirname(__FILE__).'/../../dataDB/db_local.config';
			
		}elseif(_ENVIRONMENT=='develop'){
			$this->root = dirname(__FILE__).'/../../dataDB/db_prod.config';
		}
	...
	private function assignDBData(){
		...
		$file = json_decode(base64_decode(file_get_contents($this->root)));
